...
// Custom rotation functions (handle rotation >= 32 bits)
inline uint32_t rotl32(uint32_t x, int k) {
    k &= 31;  // Reduce to [0, 31] range to avoid UB
    return (x << k) | (x >> (32 - k));
}
...
extern "C" {
    ...
    uint32_t get_cpp_canonical_hashes_scalar(
        const uint8_t* seq_data,
        uint32_t seq_len,
        uint32_t k,
        uint32_t* out_hashes,
    ...
    void debug_canonical_hash_trace(
        const uint8_t* seq_data,
        uint32_t seq_len,
        uint32_t k
    );
